...
class SamplingBatchInfo:
    ...
    logits_mask: Optional[List[List[int]]] = None
    ...
    def from_schedule_batch(cls, batch: ScheduleBatch, vocab_size: int):
        ...
        reqs = batch.reqs
        logits_mask = [req.sampling_params.logits_mask for req in reqs]
        device = batch.device
        temperatures = torch.tensor(
    ...
    def apply_logits_bias(self, logits: torch.Tensor):
        ...
        if all(i is not None for i in self.logits_mask):
            if len(self.logits_mask) != len(logits):
                aa = 4
            mask = torch.full_like(logits, float('-inf'))
            for i, idx in enumerate(self.logits_mask):
                mask[i, idx] = 0.0
            logits.add_(mask)
