...
from slime.utils.ppo_utils import (
    ...
    get_advantages_and_returns_batch1,
    get_grpo_returns,
    ...
    *,
    ...
    for i, (logits_chunk, tokens_chunk) in enumerate(get_responses(
        ...
        if loss_masks is not None:
            ...
            full_log_prob = torch.zeros(L, device=device, dtype=dtype)
...
def get_invalid_probs_loss(
    ...
    *,
    ...
    for i, (logits_chunk, tokens_chunk) in enumerate(get_responses(
        ...
        for j, idx in enumerate(logits_masks[i]):
            idx = [x - 151665 for x in idx]
            action_mask[j, idx] = 1

        invalid_probs = probs * (1 - action_mask)
        loss_invalid = invalid_probs.sum(dim=-1)

        L = response_lengths[i]

        full_loss_invalid = torch.zeros(L, device=device, dtype=dtype)
        full_loss_invalid[loss_masks[i] == 1] = loss_invalid
        loss_invalid_list.append(full_loss_invalid)

    return loss_invalid_list
...
def policy_loss_function(
    ...
    if args.advantage_estimator == "ppo1":
        ...
        invalid_probs_loss = torch.cat(invalid_probs_loss, dim=0)
        invalid_probs_loss = pg_loss_reducer(invalid_probs_loss)
        pg_loss = pg_loss + invalid_probs_loss
