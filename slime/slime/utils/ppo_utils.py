...
def get_advantages_and_returns_batch1(
    total_lengths,
    response_lengths,
    values_list,
    rewards_list,
    ...
    with torch.no_grad():
        ...
        if cp_size > 1:
            ...
            for total_len, resp_len, v in zip(
                total_lengths, response_lengths, values_list, strict=False
            ):
...
# ------------------------------------------------------------------------------------------
        ...
        gamma_tensor = torch.zeros(B, max_len, device=device, dtype=dtype)
        lambd_tensor = torch.zeros(B, max_len, device=device, dtype=dtype)
        ...
        for t in reversed(range(max_len)):
            ...
            delta = full_rewards[:, t] + gamma_tensor[:, t] * next_value - full_values[:, t]
            lastgaelam = delta + lambd_tensor[:, t] * lastgaelam
            adv_rev.append(lastgaelam)

        temp_full_advantages = torch.stack(adv_rev[::-1], dim=1)  # [B, max_len]
        temp_full_returns = temp_full_advantages + full_values
...
#------------------------------------------------------------------------------------------
        ...
        if cp_size > 1:
            ...
            for total_len, resp_len, adv_row, ret_row in zip(
                ...
                adv_sliced = slice_log_prob_with_cp(adv_full, total_len, resp_len)
                ret_sliced = slice_log_prob_with_cp(ret_full, total_len, resp_len)

                advantages_list1.append(adv_sliced)
                returns_list1.append(ret_sliced)

        else:
            return advantages_list, returns_list

    return advantages_list1, returns_list1
